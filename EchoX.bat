...
::Check For Updates
if "%DevBuild%" neq "Yes" (
curl -g -k -L -# -o "%temp%\latestVersion.bat" "https://raw.githubusercontent.com/UnLovedCookie/EchoX/main/Files/Version"
...
start "" /MIN powershell -NoProfile -NonInteractive -Command Disable-MMAgent -mc -PageCombining;^
...
nonsackrttresiliency=disabled MaxSynRetransmissions=2 >>"%temp%\EchoLog.txt" 2>>"%temp%\EchoError.txt"
echo Netsh
...
if not exist "%temp%\restart64.exe" curl -g -k -L -# -o "%temp%\Restart64.exe" "https://github.com/UnLovedCookie/EchoX/raw/main/Files/restart64.exe"
echo Downloading EmptyStandbyList [...]
if not exist "%temp%\EmptyStandbyList.exe" curl -g -k -L -# -o "%temp%\EmptyStandbyList.exe" "https://wj32.org/wp/download/1455/"
...
for %%i in (workingsets modifiedpagelist standbylist priority0standbylist) do EmptyStandbyList.exe %%i
goto:eof
